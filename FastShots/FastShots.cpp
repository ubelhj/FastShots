...
BAKKESMOD_PLUGIN(FastShots, "Fast or Slow Goal Filter", plugin_version, PLUGINTYPE_FREEPLAY)
...
void FastShots::onLoad()
{
	...
	cvarManager->registerCvar("fsgf_enabled", "0", "Enable the Fast or Slow Goal Filter plugin", true, true, 0, true, 1)
	...
	cvarManager->registerCvar("fsgf_blue", "0", "makes the blue team take fast shots", true, true, 0, true, 1)
		.addOnValueChanged([this](std::string, CVarWrapper cvar) { blueEnabled = cvar.getBoolValue(); });

	cvarManager->registerCvar("fsgf_min_blue", std::to_string(blueMaxSpeed), "minimum shot speed allowed for blue")
		.addOnValueChanged([this](std::string, CVarWrapper cvar) {
		blueMinSpeed = cvar.getFloatValue();
			});

	cvarManager->registerCvar("fsgf_orange", "0", "makes the orange team take fast shots", true, true, 0, true, 1)
		.addOnValueChanged([this](std::string, CVarWrapper cvar) { orangeEnabled = cvar.getBoolValue(); });

	cvarManager->registerCvar("fsgf_min_orange", std::to_string(orangeMinSpeed), "minimum shot speed allowed for orange")
		.addOnValueChanged([this](std::string, CVarWrapper cvar) {
		orangeMinSpeed = cvar.getFloatValue();
			});

	cvarManager->registerCvar("fsgf_max_orange", std::to_string(orangeMaxSpeed), "maximum shot speed allowed for orange")
		.addOnValueChanged([this](std::string, CVarWrapper cvar) {
		orangeMaxSpeed = cvar.getFloatValue();
			});

	cvarManager->registerCvar("fsgf_max_blue", std::to_string(blueMaxSpeed), "maximum shot speed allowed for blue")
		.addOnValueChanged([this](std::string, CVarWrapper cvar) {
		blueMaxSpeed = cvar.getFloatValue();
			});

	cvarManager->registerCvar("fsgf_update_min_blue", std::to_string(setBlueMin), 
		"updates the blue team's min shot speed", true, true, 0, true, 1)
		.addOnValueChanged([this](std::string, CVarWrapper cvar) { 
			setBlueMin = cvar.getBoolValue(); 

			if (setBlueMin) {
				cvarManager->executeCommand("fsgf_update_min_all 0");

				cvarManager->executeCommand("fsgf_min_blue 0");
				cvarManager->executeCommand("fsgf_max_blue 220");

				cvarManager->executeCommand("fsgf_update_max_blue 0");
				cvarManager->executeCommand("fsgf_update_max_all 0");
			}});

	cvarManager->registerCvar("fsgf_update_min_orange", std::to_string(setOrangeMin),
		...
		.addOnValueChanged([this](std::string, CVarWrapper cvar) {
			...
			if (setOrangeMin) {
				...
				cvarManager->executeCommand("fsgf_min_orange 0");
				cvarManager->executeCommand("fsgf_max_orange 220");

				cvarManager->executeCommand("fsgf_update_max_orange 0");
				cvarManager->executeCommand("fsgf_update_max_all 0");
			}});

	cvarManager->registerCvar("fsgf_update_min_all", std::to_string(setBothMin),
		"updates the whole lobby's min shot speed", true, true, 0, true, 1)
		.addOnValueChanged([this](std::string, CVarWrapper cvar) {
			setBothMin = cvar.getBoolValue();

			if (setBothMin) {
				cvarManager->executeCommand("fsgf_update_min_blue 0");
				cvarManager->executeCommand("fsgf_update_min_orange 0");
	...
	cvarManager->registerCvar("fsgf_update_max_blue", std::to_string(setBlueMin),
	...
	cvarManager->registerCvar("fsgf_update_max_orange", std::to_string(setOrangeMin),
	...
	cvarManager->registerCvar("fsgf_update_max_all", std::to_string(setBothMin),
	...
	cvarManager->registerCvar("fsgf_back_wall", std::to_string(backWall), "sets back wall's location")
		.addOnValueChanged([this](std::string, CVarWrapper cvar) {
...
void FastShots::setNewSpeed() {
	...
	if (setBothMin) {
		cvarManager->executeCommand("fsgf_min_blue " + std::to_string(speed));
		cvarManager->executeCommand("fsgf_min_orange " + std::to_string(speed));
	} else if (setBothMax) {
		cvarManager->executeCommand("fsgf_max_blue " + std::to_string(speed));
		cvarManager->executeCommand("fsgf_max_orange " + std::to_string(speed));
	} else if (ballLoc.Y < 0) {
		if (setOrangeMin) {
			cvarManager->executeCommand("fsgf_min_orange " + std::to_string(speed));
